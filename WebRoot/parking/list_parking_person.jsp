...
<div style="padding: 10px" class="admin-content">
    <div class="panel panel-default">
        <div class="panel-heading">
            <span class="module-name"> parking </span>
            <span>list</span>
        </div>
        <div class="panel-body">
            <div class="pa10 bg-warning">
                <form class="form-inline" id="formSearch" action="?">
                    ...
                    <div class="form-group">IC card</div>
                    <div class="form-group">
                        type
                        ...
                        <script>
                            $(".class_type10 select-update").val($(".class_type10 select-update").attr("data-value"));
                        </script>
                    </div>
                    <div class="form-group">
                        charge amount
                    ...
                    <tbody>
                        <c:set var="i" value="0" /><c:forEach items="${list}" var="map"
                            ...
                            <tr id="${map.id}" pid="">
                                ...
                                <td>
                                    <ssm:sql var="mapparking_information4" type="find">SELECT parking_lot_name,id FROM parking_information where id='${map.location}'</ssm:sql
                                    >${mapparking_information4.parking_lot_name}
                                </td>
                                <td>${map.park_time}</td>
                                ...
                                <td align="center">
                                    <c:if test="${Query.make('outOfParking').where('parking_id',map.id).count() ==0}">
                                    <a href="outOfParking.do?ac=add&id=${map.id}">outOfParking</a>
                                    </c:if>
                                    <a href="parking.do?ac=detail&id=${map.id}">detail</a>

                                    <a href="parking.do?ac=updt&id=${map.id}">edit</a>

                                    <a href="parking.do?ac=delete&id=${map.id}" onclick="return confirm('Are you sure you want to delete? ')">delete</a>
                                    <!--qiatnalijne-->
                                </td>
                            </tr>
                        </c:forEach>
