...
(function () {
    ...
    window.UMEDITOR_CONFIG = {
        ...
        //图片上传配置区
        imageUrl: URL + "jsp/imageUp.jsp", //图片上传submit地址
        imagePath: "", //图片修正地址，引用了fixedImagePath,如有特殊需求，可自行配置
        imageFieldName: "upfile", //图片数据的key,若此处修改，需要在后台对应文件修改对应参数

        //工具栏上的所有的功能按钮和下拉框，可以在newedit器的实例时选择自己需要的从新定义
        toolbar: [
        ...
        //,initialContent:'欢迎使用UMEDITOR!'    //初始化edit器的内容,也可以通过textarea/script给值，看官网例子

        //,initialFrameWidth:500 //初始化edit器宽度,默认500
        //,initialFrameHeight:500  //初始化edit器高度,默认500

        //,autoClearinitialContent:true //是否自动清除edit器初始内容，注意:如果focus属性设置为true,这个也为真，那么edit器一上来就会触发导致初始化的内容看不到了

        //,textarea:'editorValue' // submit表单时，服务器获取edit器submit内容的所用的参数，多实例时可以给容器name属性，会将name给定的值最为每个实例的键值，不用每次实例化的时候都设置这个值
        ...
        //如果自定义，最好给p标签如下的行高，要不enter中文时，会有跳动感
        //注意这里添加的样式，最好放在.edui-editor-body .edui-body-container这两个的下边，防止跟页面上css冲突
        //,initialStyle:'.edui-editor-body .edui-body-container p{line-height:1em}'

        //,autoSyncData:true //自动同步edit器要submit的数据

        //,emotionLocalization:false //是否开启表情本地化，默认关闭。若要开启请确保emotion文件夹下包含官网提供的images表情文件夹

        //,allHtmlEnabled:false //submit到后台的数据是否包含整个html字符串
